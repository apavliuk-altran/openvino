...
void Interpolate::initSupportedPrimitiveDescriptors() {
    ...
    if (implType == impl_desc_type::ref || interpAttrs.mode == InterpolateMode::linear) {
        ...
        const bool allowFp32 = mayiuse(cpu::x64::avx2) && inputPrecision == Precision::FP32;
        const bool allowI8OrU8 = interpAttrs.mode == InterpolateMode::nearest &&
                                 ((inputPrecision == Precision::I8 && outputPrecision == Precision::I8) ||
                                  (inputPrecision == Precision::U8 && outputPrecision == Precision::U8));
        if (allowFp32 || allowI8OrU8) {
            pushDesc(LayoutType::ncsp, implType);
        }
    }
}
...
Interpolate::InterpolateJitExecutor::InterpolateJitExecutor(const InterpolateAttrs& interpAttrs,
    ...
    const bool allowI8OrU8 =
        (interpAttrs.inPrc == InferenceEngine::Precision::I8 && interpAttrs.outPrc == InferenceEngine::Precision::I8) ||
        (interpAttrs.inPrc == InferenceEngine::Precision::U8 && interpAttrs.outPrc == InferenceEngine::Precision::U8);
...
bool Interpolate::canFuse(const NodePtr& node) const {
    const bool isNearestPlanar = interpAttrs.mode == InterpolateMode::nearest &&
               interpAttrs.layout == InterpolateLayoutType::planar;
    const bool isI8OrU8 =
        (interpAttrs.inPrc == InferenceEngine::Precision::I8 && interpAttrs.outPrc == InferenceEngine::Precision::I8) ||
        (interpAttrs.inPrc == InferenceEngine::Precision::U8 && interpAttrs.outPrc == InferenceEngine::Precision::U8);

    if (!mayiuse(cpu::x64::sse41) || (isNearestPlanar && isI8OrU8) || interpAttrs.mode == InterpolateMode::linear) {
        return false;
    }
