...
class BasicConvolutionBiasAddActivationLayerTest
    ...
    static_assert(std::is_same<TConvLayerTest, ConvolutionLayerTest>::value ||
                      std::is_same<TConvLayerTest, GroupConvolutionLayerTest>::value,
...
protected:
    ...
    setUpConvolutionTestParams(const typename Traits::ConvParamSet& convParamsSet) {
        ...
        size_t numGroups = 0;
        constexpr bool isGroup = std::is_same<TConvLayerTest, GroupConvolutionLayerTest>::value;
        if (isGroup) {
            // numGroups = std::get<6>(convParams);
            numGroups = std::get<convParamsSize - 2>(convParams);
        }
