...
void Interpolate::initSupportedPrimitiveDescriptors() {
    ...
    const bool allowNearestPlanarNative = interpAttrs.mode == InterpolateMode::nearest &&
                                          ...
                                          // try one_of
                                          (inputPrecision == Precision::I8 ||
                                           inputPrecision == Precision::U8 ||
    ...
    const bool isPlanarApplied = allowPlanarFp32 || allowNearestPlanarNative;
...
void Interpolate::prepareParams() {
    ...
    auto buildExecutor = [&](const InterpolateKey& key) -> std::shared_ptr<InterpolateExecutor> {
        ...
        const bool hasNoConversion = key.nodeAttrs.inPrc == key.nodeAttrs.outPrc &&
                                     // try one_of
                                     (key.nodeAttrs.inPrc == Precision::I8 ||
                                      key.nodeAttrs.inPrc == Precision::U8 ||
                                      key.nodeAttrs.inPrc == Precision::BF16);
        const bool allowNearestPlanarNative = isNearest && isPlanar && hasNoConversion;
...
Interpolate::InterpolateJitExecutor::InterpolateJitExecutor(const InterpolateAttrs& interpAttrs,
    ...
    const bool allowNearestNative = isNearest &&
                                    ...
                                    (interpAttrs.inPrc == InferenceEngine::Precision::I8 ||
                                     ...
                                     interpAttrs.inPrc == InferenceEngine::Precision::BF16);
