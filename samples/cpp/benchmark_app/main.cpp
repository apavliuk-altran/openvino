...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        auto dumpTensor = [](const ov::Tensor& t) {
            std::cout << "type: " << t.get_element_type() << '\n';
            const auto* p = static_cast<const float*>(t.data(ov::element::Type_t::f32));
        ...
        std::cout << "================================================================================================================\n";
        const auto& irs = inferRequestsQueue.requests;
        for (const auto& ir : irs) {
            for (auto& output : compiledModel.outputs()) {
                std::cout << "-----------------------------------------------------------------------------------------------------------------\n";
                std::cout << "name: " << output.get_any_name() << '\n';
                const auto& tensor = ir->get_tensor(output.get_any_name());
                dumpTensor(tensor);
                std::cout << "-----------------------------------------------------------------------------------------------------------------\n";
            }
        }
