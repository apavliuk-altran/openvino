...
protected:
    ...
    void infer() override {
        ...
        std::map<std::string, std::pair<std::string, long long>> results_us{};
        // BaseTest::Infer();
        BaseTest::infer();
        ...
        for (const auto& name : bench_names_) {
            ...
            for (const auto& result : perf_results) {
                ...
                if (shouldAdd) {
                    found = true;
                    results_us.emplace(std::make_pair(resName, std::make_pair(result.node_type, 0)));
                }
            }
            if (!found) {
        ...
        for (int i = 0; i < num_attempts_; ++i) {
            ...
            for (const auto& result : perf_results) {
                ...
                if (result_us != results_us.end()) {
                    result_us->second.second += result.real_time.count();
                }
            }
        }
        ...
        for (const auto& el : results_us) {
            const auto& name = el.first;
            const auto& type = el.second.first;
            const auto time = el.second.second / num_attempts_;
            total_us += time;
            std::cout << std::fixed << std::setfill('0') << type << " - " << name << ": " << time << " us\n";
        }
